...
2. Найдите свой диск (например, sdb1) и скопируйте его UUID (длинный код из букв и цифр)
3. Создайте папку для монтирования, например, с названием storage в корневом каталоге `mnt`
```
...
5. В самый конец файла добавьте новую строку (используя свой UUID и путь)
```
UUID=ваш-uuid-код-здесь  /mnt/storage  ext4  defaults,noatime  0  2
```

6. Если диск отформатирован в NTFS (от Windows), вместо ext4 напишите
```
ntfs-3g
# или
lowntfs-3g
...
7. Проверьте работу (без перезагрузки). Если команда **не** выдала ошибок, значит диск примонтирован, и будет подключаться автоматически при каждом включении.
```
sudo mount -a
```


## Увеличение файла подкачки в Manjaro
Увеличить файл подкачки (swap file) в Manjaro Linux можно путем создания дополнительного файла подкачки или изменения размера существующего. В Manjaro часто используется файл подкачки, а не отдельный раздел подкачки. Процесс безопасного увеличения файла подкачки включает несколько шагов с использованием терминала.
...
3. Сделайте файл файлом подкачки Linux
```
sudo mkswap /swapfile2
```

4. Активируйте новый файл подкачки немедленно
```
sudo swapon /swapfile2
```

5. Убедитесь, что новый объем подкачки используется. В строке Swap вы увидите суммарный объем подкачки.
```
free -h
```

6. Сделайте изменения постоянными (автоматическое монтирование при загрузке) через файл `/etc/fstab`
```
sudo nano /etc/fstab
```

7. Добавьте следующую строку в конец файла и сохраните его (используйте `Ctrl+O` для сохранения и `Ctrl+X` для выхода в nano)
```
/swapfile2 none swap defaults 0 0
```

8. После перезагрузки системы новый файл подкачки будет автоматически активирован.
---
...
1. Отключите текущий файл подкачки
```
sudo swapoff /swapfile
```

2. Увеличьте его размер с помощью fallocate (например до 8G)
```
sudo fallocate -l 8G /swapfile
```

3. Повторно сделайте файл файлом подкачки Linux (это необходимо делать каждый раз при изменении размера)
```
sudo mkswap /swapfile
```

4. Установите права доступа (на всякий случай)
```
sudo chmod 600 /swapfile
```

5. Снова включите подкачку
```
sudo swapon /swapfile
```

6. Проверьте результат командой
```
free -h
```
...
1. Посмотрите текущее значение (обычно 60)
```
cat /proc/sys/vm/swappiness
```

2. Установите менее агрессивное значение (например, 10) временно
```
sudo sysctl vm.swappiness=10
```

3. Сделайте изменение постоянным через файл `/etc/sysctl.d/99-sysctl.conf` и добавьте строку `vm.swappiness=10`
...
1. Обновить зеркала и выбрать 10 самых быстрых
```
sudo pacman-mirrors --fasttrack 10
```

2. Обновить зеркала только в вашей или соседних странах (для лучшего пинга). Вы можете заменить страны на те, что ближе к вам. Использование протокола https повышает безопасность
```
sudo pacman-mirrors --country Russia,Germany,France --api --proto https
```

3. После обновления зеркал ОБЯЗАТЕЛЬНО нужно синхронизировать базу данных
```
sudo pacman -Syyu
```
...
1. Создайте список пользовательских пакетов для последующего использования
```
pamac list --foreign > ~/alien-pkgs.txt
```

2. Удалите все посторонние пакеты
```
...
5. Повторите без "--dry-run"
```
pamac update
reboot
```

6. Просмотрите список пакетов, созданных на первом шаге, и перестройте только те, которые вам действительно нужны


## Проверка, что обновления прошли успешно
1. Проверка через терминал после завершения команды
```
sudo pacman -Syu
# или 
pamac update
...
Что-то пошло не так, и обновления не были завершены успешно, если присутствуют красные надписи:
  * error;
  * failed to commit transaction (не удалось завершить транзакцию);
  * package validation failed.

2. Проверка версии установленных пакетов, например, ядро Linux
```
uname -r
```
