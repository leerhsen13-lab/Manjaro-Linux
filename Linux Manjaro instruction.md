...
1.  Настройки видеокарт (для NVIDIA, позволяет ограничить FPS глобально для всех игр или индивидуально для каждой).
...
2. Сторонние программы (если в игре нет встроенного ограничителя, можно использовать специальные утилиты)
...
`lsblk -f`
   ...
   2. Создайте папку для монтирования.  
        * Например, создайте папку storage в корневом каталоге mnt\
...
`UUID=ваш-uuid-код-здесь  /mnt/storage  ext4  defaults,noatime  0  2`
        * *Примечание*: если диск отформатирован в NTFS (от Windows), вместо ext4 напишите\
`ntfs-3g` или `lowntfs-3g`

   4. Проверьте работу.  
        * Чтобы не перезагружаться и проверить, нет ли ошибок (ошибки в fstab могут помешать загрузке системы). Если команда **не** выдала ошибок, значит диск примонтирован, и теперь он будет подключаться автоматически при каждом включении ПК.\
`sudo mount -a`
