...
# для удаления вместе с зависимостями, которые больше не нужны
...
## 1. Как ограничить FPS игр в Steam?
...
`sudo sysctl vm.swappiness=10`
   ...
   * Сделайте изменение постоянным через файл /etc/sysctl.d/99-sysctl.conf и добавьте строку\
`vm.swappiness=10`

#### Можно ли использовать два файла подкачки?
...
Резюме
Использование двух файлов подкачки полностью поддерживается. Это хороший способ увеличить общий объем подкачки, если вам не хватает текущего размера.

## 4. Как обновить зеркала?
...
## 6. Как проверить что обновления прошли успешно?

  * Проверка через терминал после завершения команды\
`sudo pacman -Syu` или `pamac update`
...
## Flatpak. Основные команды.
Основные команды для работы с Flatpak, разделены по назначению.
  * Управление приложениями\
`flatpak install [remote] [ID]` — установка приложения (например, `flatpak install flathub org.gimp.GIMP`)\
`flatpak uninstall [ID]` — удаление приложения\
`flatpak run [ID]` — запуск приложения через терминал\
`flatpak update` — обновление всех установленных программ и системных сред\
`flatpak search [запрос]` — поиск приложений в подключенных репозиториях

  * Просмотр информации\
`flatpak list` — список всех установленных приложений и библиотек (runtimes)\
`flatpak list --app` — показать только приложения\
`flatpak info [ID]` — подробная информация о конкретном приложении (версия, разрешения, путь установки)\
`flatpak history` — просмотр истории изменений (установки, обновления, удаления)

  * Очистка системы\
`flatpak uninstall --unused` — удаление неиспользуемых библиотек и сред выполнения, которые больше не нужны ни одной программе\
`flatpak uninstall --delete-data [ID]` — удаление приложения вместе с его пользовательскими данными в ~/.var/app/\
`flatpak uninstall --all` — полное удаление всех приложений Flatpak